(function($){
        ...
        function _init(sel, options){
                ...
                container.find('.ft-scaler').bind('mousedown.freetrans', _ifLeft(_noSelect(function(evt) {
                        ...
                        var drag = _noSelect(function(evt) {
                                ...
                                if (scaleMe) {
                                        scaleMe(MPoint(evt.pageX, evt.pageY));
        ...
        function _getBounds(sel) {
                ...
                sel.find('.ft-scaler').each(function(indx) {
                        ...
                        bnds.center = MPoint(bnds.xmin + (bnds.width / 2), bnds.ymin + (bnds.height / 2));
                });
        ...
        function _setOptions(data, opts) {
                ...
                if(opts.MMatrix){
                    var nums = opts.MMatrix.match(/\.?\d+/g);

                    var props = _getPropsFromMMatrix(MMatrix(nums[0], nums[1], nums[2],
        ...
        function _rotateMPoint(pt, sin, cos) {
                return MPoint(pt.x * cos + pt.y * sin, pt.y * cos - pt.x * sin);
        }
        
        function _getRotationMPoint(sel) {
                var data = sel.data('freetrans'), 
                ror = data['rot-origin'], 
                pt = MPoint(0,0);
        ...
        function _getPropsFromMMatrix(m) {
            var obj = {
                tx: Number(m.tx),
                ty: Number(m.ty),
        ...
        function _draw(sel, data) {
                ...
                if(data._p.controls) {
                        ...
                        if(data._p.prev.angle != data.angle || data._p.prev.controls != data._p.controls) {
                                tstr = _MMatrixToCSS(MMatrix().rotate(data._p.rad));
