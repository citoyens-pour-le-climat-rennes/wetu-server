...
var Potrace = (function() {
  ...
  var imgElement = document.createElement("img"),
      ...
      info = {
        ...
        alphamax: 1,
        opttolerance: 0.2,
        adaptiveThresholdC: 10,
        adaptiveThresholdWindowSize: 10
      };
  ...
  function loadCanvas() {
    ...
    if(imgElement.width > maxSize || imgElement.height > maxSize) {
      ...
      if(imgElement.width > imgElement.height) {
        width = maxSize;
        height = Math.floor(width * imgElement.height / imgElement.width);
      } else {
        height = maxSize;
        width = Math.floor(height * imgElement.width / imgElement.height);
      }
    ...
    container = document.getElementById('processed-image');
    if(container) {
  ...
  function adaptiveThreshold(ctx) {
    // adaptive threshold
    let n = info.adaptiveThresholdWindowSize; // size of the block
    let C = info.adaptiveThresholdC;
    let no2 = Math.floor(n / 2);
    let blockSize = 2 * no2 + 1;
    let nPixelsInBlock = blockSize * blockSize;

    let finalBm = new Bitmap(width, height);

    var sourceImageData = ctx.getImageData(0, 0, width, height);
    // var averageImageData = ctx.getImageData(0, height, width, height);
    // var destImageData = ctx.getImageData(width, height, width, height);

    var sourceData = sourceImageData.data;
    // var averageData = averageImageData.data;
    ...
    for (let y = 0 ; y < height ; y++) {
      for (let x = 0 ; x < width ; x++) {
        ...
        for (let xi = -no2 ; xi <= no2 ; xi++) {
          for (let yi = -no2 ; yi <= no2 ; yi++) {

            let xf = x + xi < 0 ? (x - xi) : x + xi >= width ? (x - xi) : x + xi;
            let yf = y + yi < 0 ? (y - yi) : y + yi >= height ? (y - yi) : y + yi;

            let index = xf + yf * width;

            average += sourceData[4*index];
          }
        }
        ...
        let index = x + y * width;
        let color = average - C < sourceData[4*index] ? 255 : 0;
        finalBm.data[index] = color > 0 ? 1 : 0;
        // let i = 4 * index;
        // destData[i+0] = color;
        // destData[i+1] = color;
        // destData[i+2] = color;
        // destData[i+3] = 255;
        // averageData[i+0] = average;
        // averageData[i+1] = average;
        // averageData[i+2] = average;
        // averageData[i+3] = 255;
      }
    }

    // ctx.putImageData(averageImageData, 0, height);
    // ctx.putImageData(destImageData, width, height);
  ...
  function loadBm() {
    ...
    for (i = 0, j = 0; i < l; i += 4, j++) {
      ...
      bm.data[j] = color; // (color < 128 ? 1 : 0);
      data[i+0] = color;
      data[i+1] = color;
      data[i+2] = color;
    }

    ctx.putImageData(imgdataobj, width, 0);
    adaptiveThreshold(ctx);
