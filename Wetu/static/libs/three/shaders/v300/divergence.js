...
vec2 sample_grad(vec2 vUv) {
    ...
    // float dx = texture(tDiffuse, vUv - vec2(1., 0.) / resolution).x - texture(tDiffuse, vUv + vec2(1., 0.) / resolution).x;
    // float dy = texture(tDiffuse, vUv - vec2(0., 1.) / resolution).x - texture(tDiffuse, vUv + vec2(0., 1.) / resolution).x;
    return vec2(dx, dy);
}
...
float divergence3() {
    // const float e = 0.01;
    float t = texture(tDiffuse, vUv).x;
    bool dx = texture(tDiffuse, vUv - vec2(1., 0.) / resolution).x < t && texture(tDiffuse, vUv + vec2(1., 0.) / resolution).x < t;
    bool dy = texture(tDiffuse, vUv - vec2(0., 1.) / resolution).x < t && texture(tDiffuse, vUv + vec2(0., 1.) / resolution).x < t;
